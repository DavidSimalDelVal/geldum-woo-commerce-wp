...
class Geldum_Library
{
    ...
    protected function & getResponse(&$pRequest)
    {
        ...
        if (0 < curl_errno($ch)) {
           echo '[ERROR] Failed to connect to geldum-wallet-rpc at ' . $this->host . ' port '. $this->port .'</br>';
        }
    ...
    /*
     * The following functions can all be called to interact with the Geldum RPC wallet
    ...
    {
        // If I pass 1, it will be 0.0000001 gdm. Then
...
class NodeTools
{
    ...
    public function __construct($testnet = false)
    {
      if(!testnet)
      {
        $this->url = 'http://explorer.geldum.org/';
      }
      if(testnet)
      {
        $this->url = 'http://explorer.geldum.org/';
